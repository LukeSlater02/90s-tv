...
namespace NinetiesTV
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Print("Genres from the 80s", GenresFrom80s(shows));
            Print("List of Genres", UniqueGenres(shows));
            System.Console.WriteLine("Year Span");
            YearSpan(shows);
            Print("Total Minutes Required to Watch Every Episode", AllShowEpisodesWatchTime(shows));
        ...
        **************************************************************************************************/
        ...
        {
            return shows.Where(s => s.ImdbRating < 8.0 && s.EndYear - s.StartYear > 10).OrderBy(s => s.Name).First();
        }

        // 21. Return the show with the most words in the name.
        static Show WordieastName(List<Show> shows)
        {
            return shows.OrderByDescending(s => s.Name.Replace(" ", "").Length).First();
        }
        ...
        {
            return String.Join(", ", shows.Select(s => s.Name).ToArray());
        }
        ...
        {
            string finalString = String.Join(", ", shows.Select(s => s.Name).ToArray(), 0, shows.Select(s => s.Name).ToArray().Length - 1) + ", and " + shows.Select(s => s.Name).LastOrDefault();
            return finalString;
        }
        ...
        **************************************************************************************************/
        ...
        // 3. Print the years 1987 - 2018 along with the shows that started in each year (note many years will have zero shows)
        ...
        {
            List<string> eightysGenres = new List<string>();
            var eightysShows = shows.Where(s => s.StartYear >= 1980 && s.StartYear < 1990).ToList();
            foreach (Show show in eightysShows)
            {
                foreach (string genre in show.Genres)
                {
                    eightysGenres.Add(genre);
                }
            }
            IEnumerable<string> distinctEightysGenres = eightysGenres.Distinct();
            return distinctEightysGenres.ToList();
        }
        ...
        static List<string> UniqueGenres(List<Show> shows)
        {
            ...
            IEnumerable<string> distinctGenres = Genres.Distinct();
            return distinctGenres.ToList();
        }

        static void YearSpan(List<Show> shows)
        {
            var groupedShows = shows.GroupBy(s => s.StartYear);
        ...
        static int AllShowEpisodesWatchTime(List<Show> shows)
        {
            int total = 0;
            var groupedShows = shows.GroupBy(s => s.Genres.Contains("Comedy"));
            foreach (var group in groupedShows)
            {
                foreach (var show in group)
                {
                    if (group.Key)
                    {
                        for (int i = 0; i < show.EpisodeCount; i++)
                        {
                            total += 22;
                        }
                    }
